...
def store_clean_table_accuracy_progress(df):
    ...
    table['Progress wrt EV'] = [("%.3g" % best_obj)+"%" for best_obj in df['Progress wrt MOSEK estimated LB (%)']]
    table['Progress wrt CV'] = [("%.3g" % best_obj)+"%" for best_obj in df['Progress wrt MOSEK certified LB (%)']]
    table = table.sort_values(by=['Progress wrt CV'],ascending=False)
    print(table.to_latex("stats_acc_progress.txt", index = False))
    return table
...
def store_clean_table_gap_progress(df):
    ...
    table = table.dropna()
    table = table.sort_values(by=['Cert. gap reduction'],ascending=False)
    print(table.to_latex("stats_gap_progress.txt", index = False))
...
display_agg_stats(df)
table = store_clean_table_accuracy_progress(df)
store_clean_table_gap_progress(df)
